...
static void send_housekeeping(adcs_sim_state_t* state)
{
    ...
    response[6] = ADCS_DEVICE_TRAILER_0;
    response[7] = ADCS_DEVICE_TRAILER_1;
...
static void handle_command(adcs_sim_state_t* state, const uint8_t* data, size_t length)
{
    ...
    switch (cmd_id) 
    {
        ...
        case ADCS_DEVICE_REQ_DATA_CMD:
            #ifdef ADCS_CFG_DEBUG
            printf("ADCS SIM: Processing GET_DATA command\n");
            #endif
            send_adcs_data(state);
            break;

        case ADCS_DEVICE_CFG_CMD:
            #ifdef ADCS_CFG_DEBUG
            printf("ADCS SIM: Processing SET_CONFIG command with payload 0x%08X\n", payload);
            #endif
